...
function loadAsyncProps(components, params, cb) {
	...
	let componentsArray = []
	let propsArray = []
	let needToLoadCounter = components.length // 用這個變數來記錄所有元件是否皆已完成 data fetching

	const maybeFinish = () => {
		if (needToLoadCounter === 0){
			// 如果所有元件皆已完成資料載入，回呼原主人，例如 loadPropsOnServer()
	...
	// loop 每個元件，操作它身上的 loadProps()，並提供 cb()供它回報完成操作
	components.forEach((Component, index) => {
		// console.log( '\n處理一個元件: ', index, Component )
...
// jx patched: added customProps as new argument
export function loadPropsOnServer({ components, params, customProps }, cb) {

	// jx: 由於下面就直接呼叫全域 fn 了，因此在那之前必需先 patch params 將 customProps 塞進去
	params = { ...params, customProps }
...
class AsyncProps extends React.Component {
	...
	loadAsyncProps(components, params, location, options) {
		...
		// jx: 將 redux store 傳進去，這樣每個元件的 loadProps() 內就能操作 actions
		params = {...params, customProps: this.props.customProps}
